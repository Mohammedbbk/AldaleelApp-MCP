...
const startMCPServers = async () => {
  logger.info('[startMCPServers] Starting execution...'); 
  try {
    ...
    for (const server of servers) {
      ...
      console.log(`[startMCPServers] Starting ${server.name}...`); 
      
      const options = server.env ? { env: { ...process.env, ...server.env } } : {};
      logger.info(`[startMCPServers] Options for ${server.name}: ${JSON.stringify(options)}`); 
      logger.info(`[startMCPServers] Command for ${server.name}: ${server.command}`); 
    ...
    if (env.WAIT_FOR_SERVERS) {
      ...
      const healthCheckPromises = servers
        .filter(server => { 
          ...
          return shouldCheck && server.port && server.healthCheckPath; 
        })
        .map(server => { 
